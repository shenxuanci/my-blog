...
- 深读源队列现行安排见 `docs/news_source_roadmap.md`：阮一峰和 Noahpinion 已通过人工抽查并保留；Marginal Revolution 已启用，须先通过 GitHub Actions 出口验证再抽查 3 篇（不再等固定有效日数）；晚点虽已续期叶子证书，项目锁定的标准 CA 包仍无法闭合其新 TLS 信任链，继续停用；Apricitas 与 Kyla 因作者停更判死销账。
