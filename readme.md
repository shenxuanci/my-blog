...
- 深读源队列现行安排见 `docs/news_source_roadmap.md`：阮一峰和 Noahpinion 已通过人工抽查并保留；Marginal Revolution 的 Actions 出口和抓取链路正常，但旧 `topic_filter: finance` 曾将不含经济机制的 AI 同行评审文放行且实际入选，已于 2026-08-12 停用；确定性闸门已修复该缺口，仍须等修复进入 `main` 并通过 Actions validate 后再单独复议启用。晚点虽已续期叶子证书，项目锁定的标准 CA 包仍无法闭合其新 TLS 信任链，继续停用；Apricitas 与 Kyla 因作者停更判死销账。
