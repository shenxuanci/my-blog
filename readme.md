...
- **公开路径当前只到这一层**：`config.yaml` 的 `objectivity.mode` 默认 `interim`，新闻正文只启用上述 prompt 规则和原有的 support-only 事实支撑审计；唯一例外是周综述在写入前始终使用有界日报证据自动初审、定向修复和复审。完整正文取证、独立证据链佐证、新闻客观性定向修复/降级/降档的代码已经存在，但只在 shadow 或未来的 `active` 模式下运行；**`active` 尚未启用，线上验收尚未完成**。审计模型可用 `config.yaml` 的 `audit_llm` 段单独指定，留空则继承 `llm`。
...
- 切 `active` 前必须通过固定 45 条夹具的三轮客观性门与同模型、同价格的改动前后三轮配对成本门、7 天线上客观性 shadow 门、5 个有效日文字质量门，并另行人工评审；安全边界与标签接受集的决策见 `docs/adr/0005-objectivity-label-accepted-sets.md`。成本配对以 enrich 与内容审计／修复合计为准：调用数不得增加，三轮加权 token 成本不得高于基线；价格未知、截断、余额不足或运行不完整均不得通过。DeepSeek 上曾两次通过夹具三轮门——2026-07-28 的 Run #30349424143 与 2026-08-06 从 `main@a135d84` 的 Run #31027819706（红线 0、标签 93.33%、归因 100%、结构 100%，且配对成本门调用数与标准化费用双降）——但两者的共享运行时指纹此后都已被生产改动取代，只作历史证据；当前运行时必须重新执行全部门，`objectivity.mode` 保持 `interim`。
...
- 选材改革与可信轨迹已随 PR #16 于 2026-07-22 合并公开，但五门线上验收尚未通过。共享运行时指纹变化会使全部五门重新起算，而指纹覆盖 `daily_news.py`、`rollout_validation.py`、`article_extractor.py`、`requirements.txt` 加运行环境投影——**改这四个文件里的任何一个就会重置窗口，所以文档不钉指纹值，当前有效值只从 Issue #15 最新台账行的 `fingerprints.runtime` 读**。最近两次重置：2026-08-09 的 `c9e6874`／`cf8ad85`（指纹 `6ddea22e1061` → `554ec8975549`），以及 2026-08-10 的 `0af1b8c`（周综述审修改动 `daily_news.py`，下一次 publish 生效）。逐门计数是易变运行状态，本文件不复制快照，**只以 GitHub Issue #15 的幂等台账为准**；读计数前先比对指纹，跨指纹的日数不能连起来算。轨迹每天的全量初验复用 `audit_llm` 连接/模型配置并强制 `temperature: 0`；模型、schema 或评审基础设施异常一律记 `needs_review`，留给人工最终确认，不猜测通过。
...
- enrich 门被拆成两半：机械的 `removed_fields / enrichment_audited_events` 安全指标由台账自动判（超过基线 1.2 倍即 `fail`），其中分母是实际进入事实支撑/客观性审计的读者可见事件数，不能用多来源事件的凝聚度审计数 `audited_events` 代替。历史 interim 记录会从同日 `daily/<date>.js` 的 `stats.pick_count` 补全分母；窗口前不足三个有效新口径记录时只报 `needs_review`。三项文字质量检查仍然只能人工判，因此机械指标未越线也不会自动通过。管线每天按日期确定性地从每个非空类目抽 1 条、最多 5 条；`rollout-evidence-v2` 在内部 artifact 的 `enrich_review_cases` 保存这些条目的最终公开字段、每例最多 5 条来源标题和每条至多 400 字证据片段，并要求与抽样 id 一一对应。超过来源上限的轨迹或 enrich 案例都必须在调用 Judge 前拒绝。原有 `review_cases` 仍只供轨迹 Judge 使用。人工复核时用 `Rollout Manual Review` 回填：当天机械安全通过且全部抽样完成复核才记单日 `pass`，单条缺陷写入 `samples_passed` / `samples_total`，由至少 5 个有效日且窗口累计通过率 ≥80% 的门统一裁决；证据不完整时必须提交 `neutral` 且不得提交样本计数，以免错误进入通过率分母。v1 历史 artifact 不回写，也不进入新窗口人工复核；新窗口从 v2 首个有效 publish 起算。
...
- enrich 五日文字质量门、客观性 shadow 七日门 + 45 条夹具三轮门、主管线信源 14 日观察门仍然独立有效，不被选材/轨迹计数替代。详情合同与共享运行时变更后，五个线上门从首个新指纹有效 publish 重新起算，45 条夹具也必须按当前运行时重跑；enrich 安全基线取新窗口前 3 个输出日中位数。深读源队列现行安排见 `docs/news_source_roadmap.md`：阮一峰和 Noahpinion 已通过人工抽查并保留；Marginal Revolution 已启用，须先通过 GitHub Actions 出口验证，再从首个有效生产产出日起算 7 个有效日；晚点虽已续期叶子证书，项目锁定的标准 CA 包仍无法闭合其新 TLS 信任链，继续停用；Apricitas 与 Kyla 因作者停更判死销账。
