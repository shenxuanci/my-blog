...
后台选图后会立即提交图片资产；放弃文章不会自动删除已经上传的文件。上传接口校验 Base64 编码和文件头与扩展名的一致性，目标文件名包含北京时间秒级时间戳、原文件名 slug 和内容摘要，避免同秒同名的不同图片撞路径。相同内容只在相同用途和目录范围内复用；旧图片路径不迁移，也不跨用途清理。

### 后台编辑约束

- Word 粘贴按纯文本导入并转换为 Markdown 段落；Markdown 导入只规范化顶层普通正文的单换行。
- Markdown 导入只会剥掉能解析为 YAML、且至少包含一个已知文章元数据字段的开头 Front Matter；空块、无关字段、无效 YAML 与正文分隔线都原样保留。
- 导入后正文开头的第一个 `# H1` 会填进空标题框；标题框已有内容时，该 H1 原样留在正文里，不会被删除。
- API 的 `article.content` 始终表示正文；Front Matter 只在文件导入时识别一次。直接粘贴到正文框中的 `---` 块按正文保存，避免重复识别导致删段。保存和重新打开会保留代码缩进、行末空格。
...
- Front Matter 的 YAML 锚点与引用在编辑受控字段时保持可解析；若更新后无法解析，服务端拒绝写入。单篇元数据无效时，列表仍显示该文章并禁用编辑，其他文章可照常管理。
- 编辑和删除必须提交打开文章时的 GitHub blob SHA；远端已经变化时返回 `409`，刷新后重新处理，不能覆盖新版本。
- GitHub 未返回完整、可解码的文件内容或目录清单时后台会报错，不会把异常响应当作空文件或空列表。超过 1 MiB 的文章仍显示在列表中，但禁用编辑；保存时按最终 Markdown 文件字节数检查相同上限，应在本地处理超大文章。
...
站点设置中的展示文本按纯文本保存。导航名和 YAML 文本字段的校验属于构建安全边界，不要绕过后台校验直接写入不受支持的引号、反斜杠、控制字符或换行结构。
...
修改后台页的内联脚本或样式时，须按 HTML 解析后的 LF 换行计算 SHA-256/Base64，并同步更新 `vercel.json` 中 `/admin` 与 `/admin/(.*)` 两条 CSP；`npm run test:news` 会校验哈希，否则部署后的后台资源会被拦截。
